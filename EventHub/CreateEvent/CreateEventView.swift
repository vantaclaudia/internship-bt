        ...
        ZStack {
            VStack(spacing: 10) {
                ScrollView(showsIndicators: false) {
                    EventHead() {viewModel.goToHome()}
                        .padding(.top, 55)
                    PhotoPicker(image: $viewModel.image)
                        .padding(.top, 70).padding(.bottom, 73)
                    GenericInput(placeholder: "Nume eveniment", icon: "pencil", errorMessage: "", isSecure: false, text: $viewModel.eventName)
                    DateTimePicker(placeholder: "Dată eveniment", date: $viewModel.date)
                        .padding(.top, -3)
                        .padding(.bottom, -3)
                    TimePicker(placeholder: "Oră eveniment", date:  $viewModel.date)
                        .padding(.bottom, -13)
                    GenericInput(placeholder: "Locație eveniment", icon: "pin", errorMessage: "", isSecure: false, text: $viewModel.placeName)
                        .disabled(true)
                        .padding(.top, 10)
                    Participants(placeholder: "Număr de participanți", icon: "user", text: $viewModel.participants)
                        .padding(.top, -21)
                    PureTextInput(placeholder: "Descriere eveniment", icon: "file", errorMessage: "", isSecure: false, text: $viewModel.description)
                        .padding(.top, -3)
                    CustomPurpleButton(buttonText: "CREEAZĂ EVENIMENT") {viewModel.addEventToDB()}
                        .padding(.top, -35)
                }
            }
